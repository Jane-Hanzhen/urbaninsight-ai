...
UrbanInsight AI 是一个以地图为核心的 AI 城市决策助手，用于探索和比较伦敦行政区。它将空间探索、可复现的统计证据、AI 解读、上下文比较和报告生成整合为一条连续的产品工作流。
...
- 带上下文的连续追问
- 行政区之间的对比分析
- 经过 Schema 校验的结构化对话与比较响应
- AI 思考反馈、可恢复错误与一键重试
- 不重新生成 AI 内容的对话 PDF 导出
- 基于已存储区域证据动态生成的 Mock 对话与比较结果
...
初始洞察、连续追问和区域比较都使用经过 Schema 校验的响应模型，前端将其呈现为结构清晰的证据卡片，而不是未经组织的模型长文本。Prompt 明确区分证据与解读；行政区上下文在服务端重新构建；Provider 错误在到达前端之前会被清理。AI 思考态用于反馈非流式请求的等待过程；对话或比较失败后可以重试，且不会丢失当前已完成的区域分析。
